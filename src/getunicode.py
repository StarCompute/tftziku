...
#     # str_fmt+="0x"+s.encode("unicode_escape").decode().replace('\\u','')
#     str_fmt+= s.encode("unicode_escape").decode()
# # str_fmt=e.replace('0x','')
head_unicode_content = strlen_hex_fmt+head_fontsize_fmt + str_fmt
# print(head_content)
# print(len(strcontent))
...
for s1 in displatstr:
    ...
    # im.save("d:/d321.bmp")
    str_pix_content += str(getPixsDataFromImg(im))
...
# font_content=str_pix_content
# font_content=head_unicode_content
...
def genPixDataFromHexData(str, size):
    ...
    for c in range(int(l/2)):
        ...
        rtn += bin(int(childstr, size)).removeprefix('0b').rjust(8, '0')
        if (c % 2 == 1):
            rtn += "\r\n"
